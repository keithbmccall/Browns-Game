...
function brownsGame(home, away) {
    if (home == browns) {
        var brownsScore = home.score() + 3; //homefield advantage
        var awayScore = away.score();

        browns.totalPoints += brownsScore;
        away.totalPoints += awayScore;
        if (brownsScore == awayScore) {
            brownsScore += Math.floor(Math.random() * 6);
            awayScore += Math.floor(Math.random() * 6);
            if (brownsScore == awayScore) {
                browns.ties++;
                away.ties++;
            } else if (brownsScore > awayScore) {
                browns.wins++;
                away.losses++;
            } else if (brownsScore < awayScore) {
                browns.losses++;
